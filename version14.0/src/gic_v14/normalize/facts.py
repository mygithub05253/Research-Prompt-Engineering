...
def _map_metric(record: dict[str, Any]) -> str | None:
    ...
    if account_nm in {"매출원가"}:
        return "cogs"
    if account_nm in {"매출총이익"}:
        return "gross_profit"
    if "profitloss" in account_id or account_nm in {"당기순이익", "당기순이익(손실)", "분기순이익", "반기순이익"}:
        return "net_income"
    if account_id.endswith("_assets") or account_nm in {"자산총계"}:
        return "total_assets"
    if account_id.endswith("_equity") or account_nm in {"자본총계"}:
        return "total_equity"
...
def _metric_group(metric_name: str) -> str:
    if metric_name in {"revenue", "operating_income", "cogs", "gross_profit", "net_income"}:
        return "income_statement"
    if metric_name in {"operating_cash_flow", "capex"}:
        return "cash_flow"
    if metric_name in {"cash", "total_debt", "total_assets", "total_equity"}:
