# GIC 기업 리서치보고서 — 정식 프롬프트 시스템 v10.0

> **버전**: v10.0
> **작성일**: 2026-05-09
> **대상**: Gachon Investment Club (GIC) 4기 — 비전공자 부원 포함
> **호환 AI 도구**: Claude · ChatGPT · Gemini · Perplexity (웹 챗봇 복붙 100% 완결)
> **변경 요지 (v9.0 → v10.0)**: v9.0 본문 100% 보존 + **부록 C (GIC 양식 ver2 슬라이드 명세) 추가** — 챗봇 복붙으로 받은 마크다운을 PowerPoint 양식.pptx에 그대로 매핑 가능. 자동화 트랙 1(`/gic-research` Skill)과 1:1 동일 변수 사용.
...
- `GIC_위클리투자리포트_v10.md` — 4페이지 압축 위클리 (부록 C 위클리 명세 포함)
- `GIC_산업TopPick_v10.md` — 산업 진단 + Top Pick 1~2개 (부록 C TopPick 명세 포함)
- `GIC_사용설명서_v10.md` — 진입 가이드 + 데이터 수집 + 자동화 티어 1·2·3·4·5 + FAQ + 변경이력
- `../design/GIC_design-guide_v10.md` — GIC 디자인 시스템 (학회 양식 토큰, PPTX 호환성 룰, 슬라이드 좌표)
- `../design/GIC_양식_ver2.pptx` — 학회 표준 양식 원본 (트랙 1 자동화 입력)

---

## 부록 C — GIC 양식 ver2 슬라이드 명세 (v10.0 신규)

> **목적**: 본 프롬프트의 Step 0~8 결과 마크다운을 학회 양식.pptx 9페이지에 **변수 단위로 1:1 매핑**할 수 있도록 명세화. Gemini·ChatGPT 부원이 챗봇 복붙으로 받은 결과를 PowerPoint 양식.pptx에 텍스트만 옮겨도 학회 표준 양식과 동일하게 완성됨. Claude Code 부원은 `/gic-research` Skill 호출 시 동일한 변수가 자동 채워짐 (챗봇 복붙 ≡ Skill 자동화 동등성).
>
> **양식 정확 좌표**: `../design/GIC_design-guide_v10.md` §12 참조.
> **컬러 토큰**: Primary Navy `#072A51` / Accent Orange `#E57728` / Check Point Orange `#E97132` (학회 양식.pptx 추출 정확 hex)

### C.1 슬라이드 1 — 표지 (가장 복잡, 18개 도형)

```yaml
# 표지 변수 명세 — 챗봇이 이 yaml 형식으로 출력하면 양식.pptx에 그대로 매핑
slide_1_cover:
  종목명: "[기업명]"                              # 예: 삼성전기 / Display 36pt+
  종목코드: "[종목코드]"                          # 예: 009150 / 6자리
  부제: "GIC여 긱스럽게 도전하라"                  # 고정값
  작성일: "[YYYY.MM.DD]"                         # 분석 기준일
  학회기수: "GIC [N]기 [작성자명]"                # 예: GIC 4기 홍길동
  투자의견: "[BUY/HOLD/SELL]"                    # Step 7 결과
  투자의견_한글: "[매수/보유/매도]"               # 투자의견과 1:1 대응
  ...
  주가_차트:                                       # 이미지 영역
    파일경로: "[기업명]_주가차트_YYYYMMDD.png"      # 네이버금융 캡처 권장
    기간: "[2020.01 ~ YYYY.MM]"

  check_point:                                     # 표지 핵심 슬로건
    슬로건: "[≤30자, 분석의 한 줄 결론]"           # 예: "AI 서버 MLCC 1위, 멀티플 디스카운트 해소"

  point_1:                                         # ≤200자, Step 5 투자포인트 1
    제목: "[Point 1 제목 ≤25자]"
    내용: "[≤200자, 핵심 투자 논리]"
  ...
  재무지표_표:                                       # 5개년 (FY1~FY5 또는 직전2 + 추정3)
    헤더: ["12월 결산", "매출액(십억원)", "영업이익(십억원)", "지배순이익(십억원)", "PER(배)", "ROE(%)", "PBR(배)", "EV/EBITDA(배)", "DY(%)"]
    행:
      - ["FY1 (2024)", "###", "###", "###", "##.#", "##.#", "#.##", "##.#", "#.#"]
      - ["FY2 (2025)", "###", "###", "###", "##.#", "##.#", "#.##", "##.#", "#.#"]
      - ["FY3 (2026E)", "###", "###", "###", "##.#", "##.#", "#.##", "##.#", "#.#"]
...
### C.2 슬라이드 2 — CONTENTS (목차)

```yaml
slide_2_contents:
  우상단_기업명: "[기업명]([종목코드])"            # 모든 본문 슬라이드 공통
  목차:
    - 섹션: "산업분석"
      페이지: 1
      서브불릿: ["[≤25자 핵심 메시지 1]", "[≤25자 메시지 2]"]
    - 섹션: "기업분석"
      페이지: 2
      서브불릿: ["[≤25자 메시지 1]", "[≤25자 메시지 2]"]
    - 섹션: "투자포인트"
      페이지: 3
      서브불릿:
        - "Point 1. [Point 1 제목 ≤20자]"
        - "Point 2. [Point 2 제목 ≤20자]"
        - "Point 3. [Point 3 제목 ≤20자]"
    - 섹션: "투자리스크"
      페이지: 4
      서브불릿: ["[≤25자 Risk 1]", "[≤25자 Risk 2]"]
    - 섹션: "밸류에이션"
      페이지: 5
      서브불릿: []
```

### C.3 슬라이드 3 — 산업분석 (Step 3 결과)

```yaml
slide_3_industry:
  섹션_헤더: "산업분석"                              # 고정값
  첫_문단:
    제목: "[제목 문장 ≤25자]"                       # 예: "글로벌 AI 서버 시장 5년 CAGR 38%"
    설명: "[≤200자 문단 설명, Step 3-1 시장 규모·CAGR + 출처]"
  둘째_문단:
    제목: "[제목 문장 ≤25자]"                       # 예: "Porter 5 Forces — 진입장벽 高"
    설명: "[≤200자 문단 설명, Step 3-2 산업 구조·트렌드 + 출처]"
  차트_좌:
    파일경로: "[산업_시장규모_차트.png]"             # TrendForce/Yole 등
    출처: "자료: [TrendForce 2025.Q1]"
  그림_우:
    파일경로: "[산업_밸류체인_이미지.png]"           # Mermaid PNG 또는 다이어그램
    출처: "자료: [출처명]"
```

### C.4 슬라이드 4 — 기업분석 (Step 4 결과)

```yaml
slide_4_company:
  섹션_헤더: "기업분석"
  첫_문단:
    제목: "[제목 문장 ≤25자]"                       # 예: "MLCC + 카메라 모듈 양대 사업 구조"
    설명: "[≤200자, Step 4-1 사업 구조 + Moat 핵심]"
  둘째_문단:
    제목: "[제목 문장 ≤25자]"                       # 예: "삼성전자 + AI 서버 OEM 고객 다변화"
    설명: "[≤200자, Step 4-2 핵심 고객·경영진]"
  다이어그램_좌:
    파일경로: "[기업_지배구조_또는_사업구조.png]"     # Mermaid PNG
    출처: "자료: [DART 사업보고서 2024]"
  다이어그램_우상:
    파일경로: "[기업_BM_다이어그램.png]"
    출처: "자료: [기업 IR Day 2025]"
  그림_우하:
    파일경로: "[제품_이미지.png]"
    출처: "자료: [기업 홈페이지]"
```

### C.5 슬라이드 5·6 — 투자포인트 (Step 5 결과)

```yaml
# Point 1·2·3 분량에 따라 1슬라이드(슬라이드 5만) 또는 2슬라이드(5·6) 사용
slide_5_investment_points:
  섹션_헤더: "투자포인트"
  point_1:
    제목: "Point 1. [≤20자]"                        # 예: "Point 1. AI 서버 MLCC 점유율 확대"
    설명: "[≤300자, Step 5 투자포인트 1 + Evidence Card 핵심]"
    근거_표:                                          # 선택, 정량 데이터 있을 때만
      헤더: ["항목", "수치", "출처"]
      행:
        - ["FY24 점유율", "##.#%", "TrendForce 2025.Q1"]
  point_2:
    제목: "Point 2. [≤20자]"
    설명: "[≤300자]"
  point_3:                                            # 슬라이드 5에 들어가지 않으면 슬라이드 6으로
    제목: "Point 3. [≤20자]"
    설명: "[≤300자]"
```

### C.6 슬라이드 7 — 투자리스크 (Step 5 리스크 결과)

```yaml
slide_7_risks:
  섹션_헤더: "투자리스크"
  risk_1:
    제목: "Risk 1. [≤20자]"                          # 예: "Risk 1. 중국 MLCC 저가 공세"
    설명: "[≤250자, 리스크 시나리오 + 영향 정량화]"
    이미지:
      파일경로: "[리스크_차트_또는_표.png]"           # 선택
      출처: "자료: [출처명]"
  risk_2:
    제목: "Risk 2. [≤20자]"                          # 예: "Risk 2. AI 서버 수요 급랭"
    설명: "[≤250자]"
```

### C.7 슬라이드 8 — 밸류에이션 (1) (Step 6 결과)
...
slide_8_valuation_1:
  ...
  재무지표_표:                                          # 5개년 IS + 핵심 비율
    ...
    행:                                                 # FY1~FY5 (직전2 + 추정3)
      ...
      # ... 5행
  차트_좌상:
    파일경로: "[매출_영업이익_5개년_추이.png]"           # 막대그래프
    제목: "매출/영업이익 추이"
    출처: "자료: 자체 추정"
  차트_우상:
    파일경로: "[ROE_ROIC_5개년_추이.png]"
    제목: "ROE/ROIC 추이"
    출처: "자료: 자체 추정"
  차트_좌하:
    파일경로: "[멀티플_피어_비교.png]"                 # 라인 또는 막대
    제목: "PER/EV/EBITDA 피어 비교"
    출처: "자료: FnGuide / 자체 추정"
  차트_우하:
    파일경로: "[시나리오_매트릭스_또는_매출구성.png]"
    제목: "시나리오별 목표주가" 또는 "매출 구성 비중"
    출처: "자료: 자체 추정"
```

### C.8 슬라이드 9 — 밸류에이션 (2) + Compliance Notice

```yaml
slide_9_valuation_2:
  섹션_헤더: "밸류에이션"
  결론_표:                                              # 1페이지 요약 표
    헤더: ["방법론", "Bear", "Base", "Bull", "가중 평균"]
    행:
      - ["PER 적용", "[목표가]", "[목표가]", "[목표가]", "[목표가]"]
      - ["EV/EBITDA 적용", "[목표가]", "[목표가]", "[목표가]", "[목표가]"]
      - ["PBR 적용", "[목표가]", "[목표가]", "[목표가]", "[목표가]"]
      - ["DCF (선택)", "[목표가]", "[목표가]", "[목표가]", "[목표가]"]
      - ["최종 목표주가", "[]", "[]", "[]", "**[###,###원]**"]
  compliance_notice:                                    # 학회 표준 고정 텍스트
    - "본 문서는 가천대학교 금융투자 동아리 GIC의 학생들이 학습 및 연구 목적으로 작성한 리포트입니다."
    - "본 문서는 증권, 금융 상품, 기타 투자 관련 자산에 대한 매수, 매도, 또는 보유를 권유하기 위한 목적으로 제작되지 않았습니다."
    - "본 보고서에 포함된 모든 정보는 보고서 작성 시점의 시장 데이터와 공시 자료 등 공개된 정보를 바탕으로 하며, 그 정확성과 완전성을 보증하지 않습니다."
    - "시장 상황 및 기업의 경영 환경은 예측과 다르게 변동될 수 있습니다."
    - "모든 투자 결정은 투자자 본인의 판단과 책임하에 이루어져야 합니다. 따라서 본 보고서에서 제시된 의견이나 예측에 의존하여 발생한 어떠한 투자 손실에 대해서도 작성자 및 동아리는 법적, 도의적 책임을 부담하지 않습니다."
    - "본 보고서에 대한 지적재산권은 가천대학교 금융투자동아리 GIC에 있으며 어떠한 경우에도 법적 책임소재의 증빙자료로 사용될 수 없습니다."
```
...
````
[Step 0~8 정상 완료 후 추가 요청]

위 분석 결과를 부록 C 슬라이드 명세에 맞춰 yaml 형식으로 출력하라.
모든 [대괄호] 자리표시자를 실제 값으로 채우고, 글자 수 룰(≤25자·≤200자 등)을 엄격히 준수.
출처가 없는 수치는 "Data unavailable"로 표기 (블록 J).

[블록 E 적용] 의심 수치에 [검증 필요] 태그.
[블록 J 적용] 모든 수치에 (출처: OOO, YYYY.MM).

출력은 외부 4중 백틱 펜스 안에 yaml 1개 — 슬라이드 1~9 모두 포함.
````

→ 받은 yaml을 PowerPoint 양식.pptx에 슬라이드별로 텍스트 박스를 클릭해 변수 값을 옮겨 붙임.
→ 1슬라이드당 평균 3~5분, 9슬라이드 30~45분.

### Claude Code Skill 부원 (티어 3)
...
[Skill이 yaml을 자동 파싱 → 학회양식.pptx 슬라이드 마스터에 변수 자동 주입 → PPTX 출력]
````
...
- 동일 종목·동일 yaml에서 출발해야 함
- 결과 PPTX 시각 비교 → 텍스트·표·이미지 위치 ≥90% 일치
